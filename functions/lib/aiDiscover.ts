...
/** Upper bound on venues to verify — each costs a Places call. */
export const MAX_DISCOVERED_VENUES = 30
 ...
 */
export function buildDiscoverPrompt(profile: TravelerProfile, destination: string, guideContent: string): string {
  const audienceRule =
  ...
  return [
    ...
    `- List up to ${MAX_DISCOVERED_VENUES}, ordered best-first for this traveler.`,
 ...
 */
export function normalizeDiscoveredVenues(raw: unknown): DiscoveredVenue[] {
  ...
  for (const entry of venuesRaw) {
    ...
    if (out.length >= MAX_DISCOVERED_VENUES) break
  }
  return out
}
